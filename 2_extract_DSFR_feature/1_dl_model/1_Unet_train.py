...
#數據讀取
#txt_name = r'/media/root/002HardDisk2/xxt/new/train_txt/train1'
PathH5 = r'/media/ds/新加卷/xxt/bc/data_set/C6/xxt0928/331_0.8_0.8_0.8_small_nor_slice/'
listFileH5 = []
...
################################
#保存模型
result_save_path = os.path.join('/media/root/新加卷/xxt/bc/try1009/0.8_0.8_0.8_smallimg112_nor_unet/C6_0.001/0925')
...
if __name__ == "__main__":
 ...
 ##train_network
    liuzhou =False
    model = twoDNet_xxt((64, 64))
    model.compile(optimizer=Adam(lr=lr), loss=focal_loss_fixed2,metrics=['acc', precision, recall, fmeasure, tp, fp, tn, fn, yt_sum])
    #model.load_weights('/media/ds/新加卷/xxt/bc/reault/smallimg112_unet/0.001/model/BC_model-182.h5',by_name=True)
    #model = model.cuda
    # iteration counter
    num_count = 0
    batch_count = 0
 ...
 #  创建Excel表并写入数据
    ...
    for epoch in range(500):
        num_count = 0
        print(str(epoch))
        # model_name = listModel[m_n]
        # fullpath = model_s1_alone_save_Path + model_name
   ...
   #         data_c = data_c.cuda
    #        label_c = label_c.cuda
        ...
        #txt2.write('Model:'+str(model_name) +'recall:' + str(recall1) + ' precision:' + str(precision1) + ' dsc:' + str(dsc1) + '\n')