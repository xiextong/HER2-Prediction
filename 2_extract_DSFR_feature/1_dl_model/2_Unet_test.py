...
lr = 10e-4
state = "test_out"###choose 'validation' or 'test'###############
###############################################

# the path control center
#數據讀取路径
PathH5 = r'/media/root/新加卷/xxt/bc/data_set/C6/219class_0.8_0.8_0.8_small'
#保存结果路径
result_path = os.path.join('/media/root/新加卷/xxt/bc/reault/0.8_0.8_0.8_smallimg112_nor_unet/C6_0.001/ori/class_result/26/test_result/',state+'_'+Version)
if os.path.exists(result_path) == 0:
    os.makedirs(result_path)
#读取测试/验证集患者编号
listPatient = []
all_model = []
model_path = '/media/root/新加卷/xxt/bc/reault/0.8_0.8_0.8_smallimg112_nor_unet/C6_0.001/ori/model'
img_save_Path_root = '/media/root/新加卷/xxt/bc/reault/0.8_0.8_0.8_smallimg112_nor_unet/C6_0.001/ori/class_result/26/test_result/img'
...
f = open('/media/root/新加卷/xxt/bc/data_set/all_label.pkl','rb')
...
#print(listFileH5)
#print(hh)
# F = open('/media/ds/新加卷/thrombus_pro/K_10_txtname/'+Kfold+'/train.txt')
# F = open("/media/ds/新加卷/XS_2Dmodel/train_txt.txt")
# line = F.readline()
# while line:
#     line = line.strip('\n')
#     listFileH5.append(PathH5+line+".h5")
#     line = F.readline()
# F.close()
print(listPatient)
...
if __name__ == "__main__":
    ...
    model = twoDNet_xxt((height, width))
    liuzhou = False
    ...
    # 结果写入文档
    #model_list = list(range(1,2))
    #model_list = [648]
    Result_save_Path = result_path
    if os.path.exists(Result_save_Path) == 0:
        os.makedirs(Result_save_Path)
    #model_name = "XS_model_20190917_"
    #for i in range(len(all_model)):
    for i in range(26,27):
    #for i in range(26, 27):
        #保存结果
        #weight_path = '/media/root/002HardDisk3/xxt/code/unet_result/good_model/XS_model_20210306_-88.h5'
        weight_path = all_model[i]
        #weight_path =r'/media/ds/新加卷/xxt/bc/reault/C6_smallimg112_nor_unet/0.001/model/BC_model-26.h5'
        #model.load_weights(result_path + Version + '/train_model/'+ model_name+str(model_list[i])+'.h5', by_name=True)
        print(weight_path)
        ...
        for m_n in range(len(listPatient)):
            ...
            for zzz in range(zz):
                ...
                # cv2.imwrite(Result_save_Path + '/' + list_FileH5[num] + '.jpg', save_img * 255)
                cv2.imwrite(img_name, save_img * 255)


                # print("img:", extract_data_name(listFileH5[num]), " iteration: ", num_count, " loss: \n", result_data)
            #print(sum(sum(sum(predi_img))))
        ...
        # 结束计时--------------------------------------------------------------------------------------------------------
        # time_end = time.time()
        # print(weight_path.split('-')[-1], " cost: ", time_end - time_start)
        #------------------------------------------------------------------------------------------------------------------
        #
        # f2 = open(Result_save_Path+'/TotalResult_dsc.txt','a+')
        # f2.write('@model:' + weight_path.split('-')[1] + '\n@recall:' + str(recall1) + '@precision:' + str(precision1) + '@dsc:' + str(dsc1) + '\n\n')
        # f2.close()
        # print('model:' + weight_path.split('-')[1] + 'recall:' + str(recall1) + ' precision:' + str(precision1) + ' dsc:' + str(dsc1) + '\n')
        ...
        wb.save(exc_name)
    # f3 = open(result_path + Version + data_name + '_data/'+'/all_model_dsc.txt','a+')
    # f3.write(model_name +str(model_list[i])+'  dsc: ' + str(dsc_all) + '\n')
    # f3.close()