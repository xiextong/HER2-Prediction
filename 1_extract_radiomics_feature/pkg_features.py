...
for pth_file in os.listdir(load_root_path):
    if 'MRI' in pth_file:
        ...
        #save_fold_path2 = os.path.join(feature_save_path, 'Features2')
        if not os.path.exists(save_fold_path1):
            os.makedirs(save_fold_path1)


        #save_mat_by_dcm_base_sio(np.array([feature]), os.path.join(save_fold_path2, pth_name + '.mat'), 'Features')
        feature_message = {}
        #f = h5py.File(os.path.join(save_fold_path1, pth_name +'.h5'),'w')   #创建一个h5文件，文件指针是f
